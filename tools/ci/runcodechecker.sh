...
function show_sum {
  CodeChecker cmd sum -a --url ${product_name} 2>/dev/null

  if [ ${CI} ]; then
    echo
    echo "### Download Artifact for more details."
  fi

  res="`CodeChecker cmd sum -a -o csv --url ${product_name} --severity high 2>/dev/null`"
  res="`CodeChecker cmd sum -a -o csv --url ${product_name} --severity high 2>/dev/null`"
  res="`echo "$res" | sed -n '$p'`"
  res=(${res/,/ })
  report_num=${res[1]}

  echo "### You have ${report_num} HIGH level reports." 
}
